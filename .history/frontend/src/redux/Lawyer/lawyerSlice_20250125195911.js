...
export const fetchLawyers=createAsyncThunk('Lawyer/fetchLawyers',()=>{
    const firm = getState().lawyer.firm;
    return axios
    .get("https://lawyer-appointment-application.vercel.app/lawyers/"+firm)
        .then((response) => {
...
export const bookSlot = createAsyncThunk("Lawyer/bookSlot", async (_, { getState, rejectWithValue }) => {
    ...
    try {
      const response = await axios.post("https://lawyer-appointment-application.vercel.app/bookSlot/"+firm, {
        lawyerId,
        slotIndex,
      });
